...
/* internal functions */
static void init_node( NODE *node )
{
    node->child = NULL;
    node->prev = NULL;
    node->next = NULL;
...
static void build_tree( const char *mtpt )
{
...
static int ar_readlink(const char *path, char *buf, size_t size)
{
    ...
    result = gnome_vfs_get_file_info(full_path, info,
				     GNOME_VFS_FILE_INFO_DEFAULT);
    if (result != GNOME_VFS_OK)
      {
	res = gvfs2fuse(result);
	goto readlink_finished;
      }

    if (info->flags != GNOME_VFS_FILE_TYPE_SYMBOLIC_LINK) {
      res = -ENOLINK;
      goto readlink_finished;
    }

    g_snprintf(buf, size, "%s", info->symlink_name);
    res = 0;
...
static int ar_getdir (const char * path, fuse_dirh_t dirh, fuse_dirfil_t dirfil)
{
  GnomeVFSResult result;
  GnomeVFSFileInfo *info;
  struct dir_trav_s dir_trav;
  int rez;
  char *full_path;
  ...
  if (result != GNOME_VFS_OK)
  {
    rez = gvfs2fuse(result);
    goto dir_finished;
  }

  if ((info->valid_fields & GNOME_VFS_FILE_INFO_FIELDS_TYPE) == 0  ||
      (info->type & GNOME_VFS_FILE_TYPE_DIRECTORY) == 0) {
    rez = -ENOTDIR;
  }

  dir_trav.dirh = dirh;
  dir_trav.dirfil = dirfil;
  gnome_vfs_directory_visit(full_path, GNOME_VFS_FILE_INFO_DEFAULT,
			    GNOME_VFS_DIRECTORY_VISIT_DEFAULT |GNOME_VFS_DIRECTORY_VISIT_LOOPCHECK ,
			    dir_trav_fun, &dir_trav);
  rez = 0;
...
static int ar_mknod(const char *path, mode_t mode, dev_t rdev)
{
  ...
  full_path = g_strdup_printf("%s/%s", mount_name, path);
  //g_debug("%s %s %x", __FUNCTION__, full_path, mode);

#if 0
  /* check this since doesn't work as intended */
  ...
  result = gnome_vfs_create(&handle, full_path, GNOME_VFS_OPEN_NONE, TRUE, mode);
  if (result != GNOME_VFS_OK)
  {
    res = gvfs2fuse(result);
    goto mknod_finished;
...
static int ar_mkdir(const char *path, mode_t mode)
{
  ...
  result = gnome_vfs_make_directory(full_path, mode);
  if (result != GNOME_VFS_OK)
  {
    res = gvfs2fuse(result);
    goto mkdir_finished;
...
static int ar_rmdir(const char *path)
{
  ...
  result = gnome_vfs_remove_directory(full_path);
  if (result != GNOME_VFS_OK)
  {
    res = gvfs2fuse(result);
    goto rmdir_finished;
...
{
  int res;
  char *full_path_from, *full_path_to;
  GnomeVFSResult result;
  GnomeVFSURI* uri_to;

  full_path_from = g_strdup_printf("%s/%s", mount_name, from);
  full_path_to = g_strdup_printf("%s/%s", mount_name, to);
  //g_debug("%s %s %s", __FUNCTION__, full_path_from, full_path_to);

  if ((uri_to = gnome_vfs_uri_new(full_path_to)) == NULL) {
    res = -ENOENT;
    goto symlink_finished;
  }

  result = gnome_vfs_create_symbolic_link(uri_to, full_path_from);
  if (result != GNOME_VFS_OK)
  {
    res = gvfs2fuse(result);
    goto symlink_finished;
 ...
 symlink_finished:
  gnome_vfs_uri_unref(uri_to);
  g_free(full_path_from);
  g_free(full_path_to);
  return res;
}
...
static int ar_chmod(const char *path, mode_t mode)
{
  ...
  result = gnome_vfs_set_file_info(full_path, info, GNOME_VFS_SET_FILE_INFO_PERMISSIONS);
  if (result != GNOME_VFS_OK)
    {
      res = gvfs2fuse(result);
      goto chmod_finished;
...
static int ar_chown(const char *path, uid_t uid, gid_t gid)
{
  ...
  info->uid = uid;
  info->gid = gid;
  result = gnome_vfs_set_file_info(full_path, info, GNOME_VFS_SET_FILE_INFO_OWNER);
  if (result != GNOME_VFS_OK)
    {
      res = gvfs2fuse(result);
      goto chown_finished;
...
static int ar_truncate(const char *path, off_t size)
{
  ...
  result = gnome_vfs_truncate(full_path, size);
  if (result != GNOME_VFS_OK)
  {
    res = gvfs2fuse(result);
    goto truncate_finished;
  }
  res=0;
...
static int ar_utime(const char *path, struct utimbuf *buf)
{
  ...
  info->atime = buf->actime;
  info->mtime = buf->modtime;
  result = gnome_vfs_set_file_info(full_path, info, GNOME_VFS_SET_FILE_INFO_TIME);
  if (result != GNOME_VFS_OK)
    {
      res = gvfs2fuse(result);
      goto utime_finished;
...
static int ar_write(const char *path, const char *buf, size_t size,
                     off_t offset, struct fuse_file_info *fi)
{
    ...
    result = gnome_vfs_write(handle, buf, size, &nwrite);
    if (result != GNOME_VFS_OK) {
      res = gvfs2fuse(result);
      return res;
    }
...
static int ar_statfs(const char *path, struct statfs *stbuf)
{
  //g_debug("%s", __FUNCTION__);
  return -ENOSYS;
}
...
/**
 * @param buf put data into this
 * @param size size of buf
 * @param offset where to start in file
 * @param fi user data (e.g. handle), ignored here
 * @return number of bytes read, -1 on error (and set errno)
 */
static int
