...
namespace {
	...
	//_ar_open_raw(const char *path, struct fuse_file_info *fi)
	{
	 ...
	 int ar_read(const char * path, char * buf, size_t size, off_t offset, struct fuse_file_info * fi) {
		...
		int ret = pthread_mutex_lock(&fs.lock);
		if(ret) {
...
int FuseWrapper::mount(const char* initArchivePath, const char* initMountPath, char* subTree){
	...
	if(!strncmp(fs.mtpt, "/dev/fd/", sizeof("/dev/fd/") - 1))
	...
	if(fs.optionsInstance.password) {
